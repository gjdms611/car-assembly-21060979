...
이 Phase는 assembly.py를 통합하지 않는다. Phase 1(CarType)/4(EnginePart)/5(BrakePart)/6(SteeringPart)처럼 "만들자마자 assembly.py에 연결"하는 strangler-fig 방식을 이 저장소 전체 리팩토링에 적용하고 있지만, `Part`는 추상 베이스일 뿐 아직 이를 상속하는 구체 부품이 없어 연결할 대상이 없다. `Part`를 실제로 사용하는 첫 통합은 Phase 4에서 `EnginePart`가 만들어지는 순간 이루어진다.
